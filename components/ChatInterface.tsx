...
const Drawer: React.FC<{
    ...
    return (
        <div className={`absolute inset-0 z-30 transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
            ...
            <div className={`absolute top-0 left-0 w-full bg-white/90 border-t border-gray-200 shadow-2xl transition-transform duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] h-[50vh] md:h-[40vh] ${isOpen ? 'translate-y-0' : '-translate-y-full'}`}>
                {children}
            </div>
        </div>
    );
...
const TradeRow: React.FC<{ trade: Trade; type: 'open' | 'closed'; onViewChart?: () => void; }> = ({ trade, type, onViewChart }) => {
    ...
    return (
        <tr className="border-b border-gray-200/50 hover:bg-gray-100/50">
            <td className="p-2 text-sm font-semibold text-gray-900">{trade.coin.symbol}</td>
            <td className="p-2"><span className={`px-2 py-0.5 text-xs font-semibold rounded-md ${isBuy ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>{trade.direction.toUpperCase()}</span></td>
            <td className="p-2 font-mono text-xs text-gray-600">{formatCurrency(trade.entryPrice)}</td>
            {type === 'closed' && <td className="p-2 font-mono text-xs text-gray-600">{formatCurrency(trade.closePrice)}</td>}
            ...
            <td className="p-2 text-center">
                 <button onClick={onViewChart} className="p-1.5 text-gray-500 hover:text-purple-600 hover:bg-gray-200 rounded-full transition-colors" aria-label={`View chart for ${trade.coin.symbol} trade`}>
    ...
    return (
        <>
            <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-4 h-full overflow-y-auto">
                <div className="flex flex-col bg-white/50 rounded-lg border border-gray-200">
                    <h4 className="p-2 text-sm font-semibold text-purple-700 border-b border-gray-200">Open Positions ({openTrades.length})</h4>
                    <div className="flex-1 overflow-y-auto">
                       <table className="w-full text-left">
                            <thead className="text-xs text-gray-500 bg-gray-50/50 sticky top-0">
                 ...
                 <div className="flex flex-col bg-white/50 rounded-lg border border-gray-200">
                    <h4 className="p-2 text-sm font-semibold text-purple-700 border-b border-gray-200">Recent History ({closedTrades.length})</h4>
                    <div className="flex-1 overflow-y-auto">
                       <table className="w-full text-left">
                            <thead className="text-xs text-gray-500 bg-gray-50/50 sticky top-0">
        ...
        <div className="p-4 overflow-y-auto h-full space-y-6">
            {PROMPT_CATEGORIES.map(category => (
                <div key={category.title}>
                    <h3 className="text-sm font-semibold text-purple-700 mb-2">{category.title}</h3>
                    <div className="flex flex-wrap gap-2">
                        {category.prompts.map(prompt => (
                            <button key={prompt} onClick={() => onSelect(prompt)} className="px-3 py-1.5 bg-gray-100 hover:bg-gray-200 text-xs text-left text-gray-700 rounded-lg transition-colors">
                                {prompt}
                            </button>
                        ))}
                    </div>
...
const StrategyDrawerContent: React.FC = () => {
    ...
    return (
        <div className="p-4 h-full overflow-y-auto space-y-6">
            <h3 className="text-lg font-bold text-purple-700">AI Trading Strategy</h3>
            <p className="text-sm text-gray-500 -mt-4">These settings influence the trades JAX simulates. This does not affect the global Pump Pipeline.</p>
...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({ allCoins }) => {
  ...
  const handleSend = async (prompt?: string) => {
    ...
    try {
        const { text: modelResponseString, context: newContext } = await runChat(fullPrompt, imageToSend);
  ...
  return (
    <div className="h-full flex flex-col bg-white rounded-lg shadow-xl border border-gray-200 overflow-hidden relative">
      <JaxCoreMenu 
              ...
              <div ref={messagesContainerRef} className="flex-grow p-4 overflow-y-auto space-y-6">
                  ...
                  {isLoading && (
                      <div className="flex justify-start items-center space-x-3">
                          ...
                          <div className="bg-gray-100 p-3 rounded-lg"><LoadingSpinner /></div>
                      </div>
                  )}
              </div>
              <div className="p-4 border-t border-gray-200">
                  ...
                  >
                      <input
                          ...
                          disabled={isLoading}
                          className="w-full flex-grow bg-white border border-gray-300 rounded-lg p-3 text-gray-900 focus:outline-none focus:ring-2 focus:ring-purple-500 transition-shadow placeholder:text-gray-500"
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                           <button
                              ...
                              className="bg-gray-200 hover:bg-gray-300 disabled:bg-gray-100 disabled:text-gray-400 disabled:cursor-not-allowed text-gray-700 font-bold p-3 rounded-lg transition-colors"
                          ...
                          <button
                              ...
                              className="bg-purple-600 hover:bg-purple-700 disabled:bg-gray-400 disabled:cursor-not-allowed text-white font-bold p-3 rounded-lg transition-colors"
                  ...
                  <div className="mt-3 flex flex-col sm:flex-row items-center justify-end gap-2">
                      <label htmlFor="strategy-mode-toggle" className="flex items-center cursor-pointer">
                          <span className="mr-3 text-sm font-medium text-gray-600">Strategy Mode</span>
                          <div className="relative">
                              ...
                              <div className={`dot absolute left-1 top-1 bg-white w-6 h-6 rounded-full transition-transform shadow-md ${isStrategyMode ? 'translate-x-6 bg-purple-500' : ''}`}></div>
                          </div>
                      </label>
                  </div>
              </div>
          </div>
          <div className="w-full md:w-2/5 lg:w-1/3 border-t-2 md:border-t-0 md:border-l-2 border-gray-200/50 flex flex-col">
              <ContextHub
                  context={contextData}
                  allCoins={allCoins}
