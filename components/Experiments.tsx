...
const StatusPill: React.FC<{ status: Experiment['status'] }> = ({ status }) => {
    ...
    switch (status) {
        case 'pending':
            return <div className={`${baseClasses} bg-gray-100 text-gray-800`}><ClockIcon className="w-3 h-3" /> Pending</div>;
        case 'running':
            return <div className={`${baseClasses} bg-blue-100 text-blue-800`}><LoadingSpinner /> Running</div>;
        case 'completed':
            return <div className={`${baseClasses} bg-purple-100 text-purple-800`}><CheckCircleIcon className="w-4 h-4" /> Completed</div>;
        default:
            return null;
    }
};
            ...
            {parts.map((part, index) => {
                ...
                if (coinMap.has(upperPart)) {
                    ...
                    if (coin) {
                        ...
                        return (
                            <span key={index} className="inline-flex items-center bg-gray-100 rounded-md px-1.5 py-0.5 mx-1 font-bold text-gray-800 align-middle">
...
const ExperimentCard: React.FC<{
    ...
    return (
        <div className="bg-white rounded-lg border border-gray-200 flex flex-col animate-fade-in-up shadow-md">
            <div className="p-4">
            ...
            <div className="mt-auto bg-gray-50 p-3 border-t border-gray-200 flex flex-col sm:flex-row justify-between items-center gap-3">
                <div className="flex-grow text-center sm:text-left">
                    {experiment.status === 'completed' ? (
                        <>
                            <p className="text-xs text-gray-500">Result (P/L)</p>
                            <p className={`text-2xl font-bold font-mono ${isProfitable ? 'text-green-600' : 'text-red-600'}`}>
                        ...
                        <>
                            {isProfitable ? (
                                isSignalActivated ? (
                                    ...
                                    <button onClick={handleActivate} className="flex items-center gap-2 px-3 py-1.5 text-sm font-semibold text-purple-800 bg-purple-100 hover:bg-purple-200 rounded-full transition-colors">
                                        <BellIcon className="w-4 h-4" /> Activate
                                    </button>
                                )
                            ) : (
                                <button onClick={handleRecycle} className="flex items-center gap-2 px-3 py-1.5 text-sm font-semibold text-yellow-800 bg-yellow-100 hover:bg-yellow-200 rounded-full transition-colors">
                                    <RecycleIcon className="w-4 h-4" /> Recycle
                                </button>
                            )}
                             <button onClick={handleResume} className="flex items-center gap-2 px-3 py-1.5 text-sm font-semibold text-blue-800 bg-blue-100 hover:bg-blue-200 rounded-full transition-colors">
...
const ActivityLog: React.FC<{ logs: LogEntry[] }> = ({ logs }) => (
    <div className="bg-white rounded-lg border border-gray-200 flex flex-col h-full shadow-md">
        <h3 className="text-lg font-bold text-purple-700 p-4 flex items-center gap-2 border-b border-gray-200">
            <ActivityLogIcon /> Activity Log
        </h3>
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {logs.length > 0 ? logs.map(log => {
                const typeColor = log.type === 'success' ? 'bg-green-500' : log.type === 'failure' ? 'bg-red-500' : 'bg-gray-400';
                return (
    ...
    return (
        <div className="w-full h-full flex flex-col bg-white rounded-lg shadow-xl border border-gray-200 overflow-hidden">
            <div className="p-4 border-b border-gray-200">
                <h2 className="text-xl font-semibold text-gray-900">AI Experiments & R&D</h2>
                <p className="text-sm text-gray-500">Tracking performance, iterating on patterns, and managing the activity log.</p>
            </div>

            <div className="flex-1 p-4 grid grid-cols-1 xl:grid-cols-3 gap-6 overflow-hidden bg-gray-50">
                <div className="xl:col-span-2 overflow-y-auto pr-2 -mr-2">
                    <h3 className="text-lg font-bold text-purple-700 mb-4">Active & Pending Experiments</h3>
                    {experiments.length > 0 ? (
                        ...
                        <div className="flex flex-col items-center justify-center h-full text-center text-gray-500">
                            <BeakerIcon className="w-12 h-12 mb-3 text-gray-400"/>
